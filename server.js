...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (pathname === '/api/tickets/promocional' && req.method === 'POST') {
      return parseBody(req, async body => {
        if (body.senha !== ADMIN_SENHA) return jsonRes(res, 401, { ok: false, error: 'Senha incorreta.' });
        const cpfLimpo = (body.cpf || '').replace(/[^\d]/g, '');
        if (!validarCPF(cpfLimpo)) return jsonRes(res, 400, { error: 'CPF inválido.' });
        const typeKey = body.typeKey || 'normal';
        const typeNames = { normal: 'Normal', vip: 'VIP', meia: 'Meia-Entrada' };
        const typeName = body.type || typeNames[typeKey] || 'Normal';
        ...
        for (let i = 0; i < qty; i++) {
          ...
          const ticket = {
            ...
            type: typeName, typeKey, price: 'GRÁTIS',
            qty: 1, pagamento: 'promo', promo: true,
            used: false, createdAt: new Date().toISOString()
          };
          await addTicket(ticket);
          tickets.push(ticket);
        }
        jsonRes(res, 201, { ok: true, tickets });
      });
    }
    ...
    if (pathname === '/api/admin/login' && req.method === 'POST') {
      return parseBody(req, async body => {
        ...
        if (senha !== ADMIN_SENHA) return jsonRes(res, 401, { ok: false, error: 'Senha incorreta.' });
